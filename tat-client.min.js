 ...
 */
Tat._tags = {
  '$': function(v){return (!v&&v!==0?'':(typeof v=='object'?stringify(v)||'':''+v)).replace(/</g,'&lt;');},
  '&': function(v){return escape(!v&&v!==0?'':''+v);}
};
  ...
  //if (!Tat._tags[tagName]) {
    if (document.registerElement) {
      ...
      Tat._fallback = function (readyElement) {
        all(readyElement, '*', populate);
      };
      onReady(Tat._fallback);
      bind(document, 'DOMNodeInserted', function (element, event, target) {
        Tat._fallback = doNothing;
        populate(target);
      });
    }
  //}
...
Tat._populateElement = function (element, template) {
  if (!element._template) {
    ...
    forEach(element.attributes, function (pair) {
      var name = pair.name;
...
Tat._proto._update = function (data) {
  var self = this;
  decorateObject(self._state, data);
  self._render();
  return self;
};
...
Tat._listenForStateEvents = function (events) {
  on('state', events, function (parent, event, element) {
    if (/^(button|input|select|textarea)$/.test(getTag(element))) {
    }
  });
};
